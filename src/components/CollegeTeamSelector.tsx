        ...
        <SelectContent style={contentStyle} className="max-h-96">
          {collegeTeams.map(team => (
            <SelectItem 
              key={team.id} 
              value={team.id}
              className="text-lg text-slate-900 dark:text-slate-100 hover:bg-blue-50 dark:hover:bg-blue-900/50 focus:bg-blue-100 dark:focus:bg-blue-900 cursor-pointer py-3 transition-all duration-200 ease-in-out hover:pl-4 transform"
            >
              {team.name}
            </SelectItem>
          ))}
